...
test("admin can run settings, connector, task, and export workflows", async ({ browser, baseURL }) => {
  ...
  await page.locator("#credential-alias").fill("grants-api");
  await page.locator("#credential-env-var").fill("GRANTS_API_TOKEN");
  ...
  await page.waitForLoadState("networkidle");
  await expect(page.getByText("EU-2026-07-19")).toBeVisible();
  ...
  await page.locator("#task-create-title").fill("Playwright created task");
  await page.locator("#task-create-assignee").selectOption("admin");
  await page.locator("#task-create-due-date").fill("2026-07-24");
  await page.locator("#task-create-description").fill("Created from the Playwright flow.");
  ...
  await page.locator("#task-edit-title").fill("Playwright edited task");
  await page.locator("#task-edit-description").fill("Updated from the Playwright flow.");
  await page.locator("#task-edit-status").selectOption("in-progress");
